...
import static ashlified.util.CardinalDirection.*;
...
public class DungeonSerializer implements Json.Serializer<Dungeon> {
  ...
  public Dungeon read(Json json, JsonValue jsonData, Class type) {
    dungeon = new Dungeon(json.readValue("grid", ArrayList.class, DungeonSection.class, jsonData));
    ...
    for (Vector3 adjacentSectionPosition : section.getAdjacentSectionPositions()) {
      ...
      if (adjacentSection != null) {
        if (adjacentSectionPosition.x < section.getPosition().x) {
          adjacentSections.put(WEST, new DefaultConnection<>(section, adjacentSection));
          section.getConnections().add(adjacentSections.get(WEST));
        } else if (adjacentSectionPosition.x > section.getPosition().x) {
          adjacentSections.put(EAST, new DefaultConnection<>(section, adjacentSection));
          section.getConnections().add(adjacentSections.get(EAST));
        } else if (adjacentSectionPosition.z < section.getPosition().z) {
          adjacentSections.put(NORTH, new DefaultConnection<>(section, adjacentSection));
          section.getConnections().add(adjacentSections.get(NORTH));
        } else if (adjacentSectionPosition.z > section.getPosition().z) {
          adjacentSections.put(SOUTH, new DefaultConnection<>(section, adjacentSection));
          section.getConnections().add(adjacentSections.get(SOUTH));
        }
      }
    }
  }
