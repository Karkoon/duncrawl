...
void main() {
		...
		vec4 diffuse = texture2D(u_diffuseTexture, v_diffuseUV); // modified
	#endif
		...
		gl_FragColor.rgb = diffuse.rgb;
			...
			#else
			    vec3 test = diffuse.rgb * v_lightDiffuse;
			    vec2 stepped = step(vec2(0.02), test.rg);
                float steppedMulti = stepped.r * stepped.g;
                gl_FragColor.rgb = test.rgb * steppedMulti;
			#endif //shadowMapFlag
